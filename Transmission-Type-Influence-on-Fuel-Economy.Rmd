...
```{r}
data(mtcars); dim(mtcars); head(mtcars,3)
```
...
Strong relation doesn't mean causation. Let's
...
and sporty (with rapid acceleration to quarter mile) ones. Reiterating the same
...
model don't have any apparent patterns in them, although they aren't ideally
